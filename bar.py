...
<Bar>:
    canvas:
        ...
        Color:
            rgba: root.color
        Rectangle:
            pos: self.pos if self.orientation in('lr','bt') else (self.right - self.width*self._value/100.0,self.y) if root.orientation =='rl' else(self.x,self.top - self.height*self.value/100.0) 
